...
describe('TextMotion', () => {
  ...
  it('defaults to span tag', () => {
    render(<TextMotion text="X" />);
    const container = screen.getByLabelText('X');
  ...
  it('applies correct animation style for each character', () => {
    ...
    expect(spans[0].style.animation).toBe('fade 0.25s ease-out 0s both');
    expect(spans[1].style.animation).toBe('fade 0.25s ease-out 0.025s both');
  });
});
